...
typedef VideoPlayerSymbolLoader =
    Pointer<T> Function<T extends NativeType>(String name);
...
class VideoPlayerProcs {
  ...
  VideoPlayerProcs(VideoPlayerSymbolLoader lookup)
    : getTextureId = lookup<NativeFunction<IntPtr Function(IntPtr)>>(
        'get_texture_id',
      ).asFunction(),
      setHttpHeaders =
          lookup<
                NativeFunction<
                  Void Function(IntPtr, Pointer<Pointer<Utf8>>, UintPtr)
                >
              >('set_http_headers')
              .asFunction(),
      load =
      ...
      pause = lookup<NativeFunction<Void Function(IntPtr)>>(
        'pause',
      ).asFunction(),
      play = lookup<NativeFunction<Void Function(IntPtr)>>('play').asFunction(),
      playlistNext = lookup<NativeFunction<Void Function(IntPtr)>>(
        'playlist_next',
      ).asFunction(),
      playlistPrev = lookup<NativeFunction<Void Function(IntPtr)>>(
        'playlist_prev',
      ).asFunction(),
      seekTo = lookup<NativeFunction<Void Function(IntPtr, Double)>>(
        'seek_to',
      ).asFunction(),
      setSpeed = lookup<NativeFunction<Void Function(IntPtr, Double)>>(
        'set_speed',
      ).asFunction(),
      setSubtitleFontSize =
