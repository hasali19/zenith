...
const BASE_URL = "https://zenith.hasali.uk";
...
export default {
  movies: {
    ...
    async getRecent() {
      const res = await fetch(BASE_URL + `/api/movies/recent`);
      const movies = await res.json();
      return movies.map((m: MovieJson) => new Movie(m));
    },
  },
  ...
  tv: {
    ...
    async getEpisodes(seasonId: ItemId): Promise<TvEpisode[]> {
      const res = await fetch(
        BASE_URL + `/api/tv/seasons/${seasonId}/episodes`
      );
    ...
    async getEpisode(id: ItemId) {
      const res = await fetch(BASE_URL + `/api/tv/episodes/${id}`);
  ...
  videos: {
    getVideoUrl(id: ItemId) {
      return BASE_URL + `/api/videos/${id}`;
    },

    async getVideoInfo(id: ItemId) {
      const res = await fetch(BASE_URL + `/api/videos/${id}/info`);
  ...
  metadata: {
    async refresh(id: ItemId) {
      await fetch(BASE_URL + `/api/metadata/${id}/refresh`, { method: "POST" });
    },
  },

  progress: {
    async update(id: ItemId, position: number) {
      await fetch(BASE_URL + `/api/progress/${id}?position=${position}`, {
        method: "POST",
      });
    },
  },
